...
test.describe('ระบบสามารถสร้างใบเสนอราคาได้', () => {
  ...
  test('เมื่อทำการกรอก "ที่อยู่สถานที่ตั้งหรือเก็บทรัพย์สินเอาประกันภัย" และตรงกับเขตที่มี Block อันตรายจะแสดงข้อความเตือนแล้วให้เลือกพื้นที่อันตราย', async ({
    ...
    await frAssuredDetailsPage.fillAddressFormWithDangerousArea();
    // กดปุ่ม "เริ่มคำนวณเบี้ย"
    await frAssuredDetailsPage.nextPage();
    // ตรวจสอบการแสดงข้อความเตือนให้ระบุพื้นที่อันตรายอีกครั้ง
    await expect(page.getByText('กรุณาระบุพื้นที่อันตราย')).toBeVisible();
  });
});