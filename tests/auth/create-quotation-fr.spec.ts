...
test.describe('ระบบสามารถสร้างใบเสนอราคาได้', () => {
  ...
  test('สามารถลบที่อยู่ของสถานที่ตั้งหรือเก็บทรัพย์สินเอาประกันภัยได้', async ({
    ...
    await frAssuredDetailsPage.fillNaturalPersonForm();
    await frAssuredDetailsPage.fillAddressForm();
    // ลบที่อยู่ผู้เอาประกันภัย (เนื่องจากหน้าเว็บไม่ได้กำหนด id หรือชื่อเฉพาะของปุ่มถังขยะ จึงใช้วิธีลบที่อยู่รายการที่สอง)
    await frAssuredDetailsPage.deleteAddress(1);
  });

  test('เมื่อทำการกรอก "ที่อยู่สถานที่ตั้งหรือเก็บทรัพย์สินเอาประกันภัย" และตรงกับเขตที่มี Block อันตรายจะแสดงข้อความเตือนแล้วให้เลือกพื้นที่อันตราย', async ({
    page,
    configuration,
    msLoginLogoutPage,
    homePage,
    ...
    // กดปุ่ม "ถัดไป" ในหน้าผู้รับผิดชอบ
    await frResponsiblePersonPage.nextPage();
    // กรอกรายละเอียดผู้เอาประกันภัยแบบบุคคล
    await frAssuredDetailsPage.fillNaturalPersonForm();
    // กรอกที่อยู่ผู้เอาประกันภัยที่ตรงกับเขตที่มี Block อันตราย
    await frAssuredDetailsPage.fillLocationFormWithDangerousArea();
  });

  test('การ Validate ข้อมูลพื้นที่อันตราย ซึ่งจะเป็น Required field ที่จำเป็นต้องกรอกทุกครั้งหากปรากฎขึ้นมาให้เลือก', async ({
    page,
    configuration,
    msLoginLogoutPage,
    homePage,
    ...
    await frAssuredDetailsPage.fillNaturalPersonForm();
    // กรอกที่อยู่ผู้เอาประกันภัยที่ตรงกับเขตที่มี Block อันตราย
    await frAssuredDetailsPage.validateLocationFormWithDangerousArea();
  });

  test.only('เมื่อทำการกรอก "ที่อยู่สถานที่ตั้งหรือเก็บทรัพย์สินเอาประกันภัย" และตรงกับเขตที่มี Block อันตรายจะแสดงข้อความเตือนแล้วให้เลือกพื้นที่อันตราย หากทำการลบไปและทำการเพิ่มที่อยู่ที่มีเขตพื้นที่อันตรายอีกครั้ง จะยังสามารถเลือกพื้นที่อันตรายได้เช่นเดิม', async ({
    page,
    configuration,
    msLoginLogoutPage,
    homePage,
    ...
    // กรอกที่อยู่ผู้เอาประกันภัยที่ตรงกับเขตที่มี Block อันตราย
    await frAssuredDetailsPage.fillLocationFormWithDangerousArea();
    // ลบที่อยู่สถานที่ตั้งหรือเก็บทรัพย์สินเอาประกันภัย แล้วเพิ่มที่อยู่ใหม่ที่ตรงกับเขตที่มี Block อันตรายอีกครั้ง
    await frAssuredDetailsPage.deleteAddress(0); //ใช้ index 0 เนื่องจากยังไม่มีการกรอกที่อยู่ของผู้เอาประกันภัย ตอนนี้จึงมีปุ่มถังขยะปุ่มเดียว
    await frAssuredDetailsPage.fillLocationFormWithDangerousArea();
  });
});